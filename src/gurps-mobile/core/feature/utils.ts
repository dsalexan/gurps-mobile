...
/**
 * Parse modifier object into a display value
 */
export function parseValue(source: never, actor: GurpsMobileActor): ModifierValue {
  let modifier = source.modifier || 0
  ...
  if (source.type === `attribute`) {
    attribute = source.attribute.toUpperCase()

    value = actor.system.attributes[attribute].value + modifier
    label = `${attribute}${parseModifier(modifier)}`
    secondary_label = `ATTR<br />`
  } else if (source.type === `skill`) {
    const skillLevel = actor.getCachedSkill(source.fullName) ?? {
      sl: `?`,
      rsl: {
        definitions: [],
        expression: `—`,
    ...
    // label = `${source.fullName}${parseModifier(modifier)}`
    label = skillLevel.rsl.toString({ skillAcronym: true })
    value = skillLevel.sl
    secondary_label = `SKILL<br />`
  } else {
    // eslint-disable-next-line no-debugger
    debugger
  }
